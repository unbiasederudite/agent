...
def test_create_chat_completion_given_message_missing_content_returns_400_with_openai_error_shape():
    client = _client_for(LLMNotFoundError("unused"))

    response = client.post(
        "/v1/chat/completions",
        json={"model": "openai/gpt-4o", "messages": [{"role": "user"}]},
    )
...
def test_create_chat_completion_given_empty_tool_calls_and_no_content_returns_400():
    client = _client_for(LLMNotFoundError("unused"))
...
def test_create_chat_completion_given_inbound_tool_calls_returns_400():
    client = _client_for(LLMNotFoundError("unused"))
...
def test_create_chat_completion_given_content_and_empty_tool_calls_returns_200():
    run = Run(
        model="openai/gpt-4o",
        request=[Message(role="user", content="hi")],
        response=Message(role="assistant", content="hello!"),
...
def test_create_chat_completion_given_tools_field_returns_200_and_passthrough_tool_calls():
    run = Run(
        model="openai/gpt-4o",
...
def test_create_chat_completion_given_unknown_tool_returns_404_with_openai_error_shape():
...
def test_create_chat_completion_given_no_tool_calls_returns_null_tool_calls_field():
    run = Run(
        model="openai/gpt-4o",
        request=[Message(role="user", content="hi")],
        response=Message(role="assistant", content="hello!"),
